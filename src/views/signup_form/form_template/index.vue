...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (this.tmpDetail.name === '' || this.tmpDetail.name.trim() === '') {
        this.$message.error('请输入报名表名称')
        return
      }
      for (let i = 0; i < this.formDetail.length; i++) {
        if (this.formDetail[i].content === '' || this.formDetail[i].content.trim() === '') {
        ...
        if (this.formDetail[i].type !== 0) {
          ...
          for (let j = 0; j < this.formDetail[i].options.length; j++) {
            if (this.formDetail[i].options[j] === '' || this.formDetail[i].options[j].trim() === '') {
